...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int bestIndexColumn = -1;
        for (int indexColumn : getEligibleIndexColumns(table)) {
            SelectPredicate sp = this.selectPredicates.get(indexColumn);
            ...
            if (cost < minCost) {
                minCost = cost;
                minOp = indexScan;
                bestIndexColumn = indexColumn;
            }
        }
        minOp = addEligibleSelections(minOp, bestIndexColumn);
        return minOp;
    }
    ...
    public Iterator<Record> execute() {
        ...
        int totoalPasses = this.tableNames.size();
        ...
        for (int pass = 2; pass <= totoalPasses; pass++) {
            prevMap = minCostJoins(prevMap, pass1Map);
            //currentPassMap = minCostJoins(currentPassMap, currentPassMap);
        }
