...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.contains(key)) {
            throw new BPlusTreeException("Duplicate key insertions are not allowed.");
        }
        ...
        if (keys.size() > metadata.getOrder() * 2) {
            List<DataBox> newRightKeys = new ArrayList<>(keys.subList(metadata.getOrder(), keys.size()));
            List<RecordId> newRightRids = new ArrayList<>(rids.subList(metadata.getOrder(), rids.size()));

            keys.retainAll(new ArrayList<>(keys.subList(0, metadata.getOrder())));
            rids.retainAll(new ArrayList<>(rids.subList(0, metadata.getOrder())));

            LeafNode sibling = new LeafNode(metadata, bufferManager, newRightKeys, newRightRids, rightSibling, treeContext);
            rightSibling = Optional.of(sibling.getPage().getPageNum());
            sync();
            return Optional.of(new Pair<>(newRightKeys.get(0), sibling.getPage().getPageNum()));
        } else {
            sync();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Optional<Long> rightSibling = pageId == -1 ? Optional.empty() : Optional.of(pageId);
