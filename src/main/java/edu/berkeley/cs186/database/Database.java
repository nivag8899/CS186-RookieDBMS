    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                ...
                for (int acquiredLocksIterator = acquiredLocks.size() - 1; acquiredLocksIterator >= 0; acquiredLocksIterator --) {
                    LockContext.fromResourceName(lockManager, acquiredLocks.get(acquiredLocksIterator).name).release(transaction);
                }
                return;
            } catch (Exception e) {
